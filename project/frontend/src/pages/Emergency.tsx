...
const EMERGENCY_SERVICES = [
  { name: 'Ambulance', number: '108', icon: 'Amb', color: 'bg-red-100 text-red-700' },
  { name: 'Police', number: '100', icon: 'Pol', color: 'bg-blue-100 text-blue-700' },
  { name: 'Fire Brigade', number: '101', icon: 'Fire', color: 'bg-orange-100 text-orange-700' },
  { name: 'Women Helpline', number: '1091', icon: 'WH', color: 'bg-purple-100 text-purple-700' },
  { name: 'Disaster Management', number: '108', icon: 'SOS', color: 'bg-yellow-100 text-yellow-700' }
];
...
export default function Emergency() {
  ...
  const handleActivateEmergency = async () => {
    ...
    try {
      await apiRequest(`/api/vehicle/${vehicleId}/emergency`, {
        method: 'POST',
        auth: false,
        body: {
          locationLat: location.lat,
          locationLng: location.lng,
          activationNote: note.trim() || null,
        }
      });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-slate-100 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
          ...
          <button onClick={() => navigate('/')} className="w-full bg-slate-800 text-white py-3 px-4 rounded-lg hover:bg-slate-700 transition-colors font-medium">
            Done
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-slate-100 p-4 py-8">
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          ...
          {gettingLocation ? (
            ...
            <div className="space-y-3">
              ...
              <button onClick={openMapsWithLocation} className="w-full flex items-center justify-center gap-2 bg-slate-100 text-slate-700 py-2.5 px-4 rounded-lg hover:bg-slate-200 transition-colors text-sm font-medium">
                <MapPin className="w-4 h-4" />
                View on Google Maps
              </button>
              <button onClick={handleGetLocation} className="w-full bg-slate-100 text-slate-700 py-2.5 px-4 rounded-lg hover:bg-slate-200 transition-colors text-sm font-medium">
                Refresh Location
              </button>
            </div>
          ) : (
            <div className="text-center py-6">
              ...
              <button onClick={handleGetLocation} className="bg-slate-800 text-white py-2.5 px-6 rounded-lg hover:bg-slate-700 transition-colors text-sm font-medium">
                Try Again
              </button>
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {EMERGENCY_SERVICES.map((service, index) => (
              <a key={index} href={`tel:${service.number}`} className={`${service.color} rounded-lg p-4 hover:opacity-80 transition-opacity`}>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-bold">{service.icon}</span>
                  <div>
        ...
        <div className="space-y-3">
          ...
          <button onClick={() => navigate(-1)} className="w-full bg-slate-100 text-slate-700 py-3 px-4 rounded-lg hover:bg-slate-200 transition-colors font-medium">
            Cancel
          </button>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
          ...
          <ul className="text-xs text-slate-600 space-y-2 leading-relaxed">
            <li>Use the vehicle QR code to quickly access public help actions.</li>
            <li>Only use this system in genuine cases.</li>
            <li>If the situation is serious, call the emergency service numbers immediately.</li>
